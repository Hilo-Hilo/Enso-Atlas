...
def test_project_slide_scope_memoizes_labels_loads_without_retired_column_path():
    src = _read_main()

    assert 'source="slides.project_id"' not in src
    retired_cache_name = "_" + "legacy" + "_project_column_exists"
    assert retired_cache_name not in src
...
def test_heatmap_endpoints_emit_structured_timing_logs():
    src = "\n".join(
        [
            _read_main(),
            (REPO_ROOT / "src" / "enso_atlas" / "api" / "model_routes.py").read_text(
                encoding="utf-8"
            ),
            (REPO_ROOT / "src" / "enso_atlas" / "api" / "heatmap_routes.py").read_text(
                encoding="utf-8"
            ),
            (REPO_ROOT / "src" / "enso_atlas" / "api" / "model_heatmap_routes.py").read_text(
                encoding="utf-8"
            ),
        ]
    )
