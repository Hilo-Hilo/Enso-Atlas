...
class TestModelPickerContainerDarkMode:
    ...
    def test_container_has_dark_bg(self):
        ...
        assert "dark:bg-navy-800" in src, "ModelPicker container must include dark:bg-navy-800"
    ...
    def test_container_keeps_light_bg(self):
        src = _src_model_picker()
        assert "bg-white" in src, "ModelPicker container must retain bg-white for light mode"
...
class TestModelPickerHeaderDarkMode:
    ...
    def test_header_hover_dark(self):
        src = _src_model_picker()
        assert "dark:hover:bg-navy-700" in src, "Header button must include dark:hover:bg-navy-700"

    def test_header_title_dark_text(self):
        src = _src_model_picker()
        # "Model Selection" label
        assert "dark:text-gray-100" in src, "Header title must include dark:text-gray-100"
    ...
    def test_chevron_dark_color(self):
        src = _src_model_picker()
        assert "dark:text-gray-500" in src, "Chevron icons must include dark:text-gray-500"
...
class TestResolutionLevelDarkMode:
    ...
    def test_inactive_resolution_dark_bg(self):
        src = _src_model_picker()
        assert "dark:bg-navy-700" in src, "Inactive resolution button must include dark:bg-navy-700"
    ...
    def test_resolution_label_dark_text(self):
        src = _src_model_picker()
        assert "dark:text-gray-400" in src, "Resolution Level label must include dark:text-gray-400"
...
class TestEmbeddingStatusDarkMode:
    ...
    def test_ready_icon_dark_color(self):
        src = _src_model_picker()
        assert "dark:text-green-400" in src, "Green check icon must include dark:text-green-400"
...
class TestForceReembedDarkMode:
    ...
    def test_checkbox_dark_border(self):
        src = _src_model_picker()
        assert "dark:border-navy-500" in src, "Checkboxes must include dark:border-navy-500"

    def test_checkbox_dark_bg(self):
        src = _src_model_picker()
        assert "dark:bg-navy-700" in src, "Checkboxes must include dark:bg-navy-700 for background"

    def test_force_reembed_label_dark(self):
        src = _src_model_picker()
        assert "dark:text-gray-300" in src, "Force Re-embed label must include dark:text-gray-300"
...
class TestQuickActionChipsDarkMode:
    ...
    def test_cancer_chip_dark_bg(self):
        src = _src_model_picker()
        assert "dark:bg-pink-900/30" in src, "Cancer-specific chip must include dark:bg-pink-900/30"

    def test_cancer_chip_dark_text(self):
        src = _src_model_picker()
        assert "dark:text-pink-300" in src, "Cancer-specific chip must include dark:text-pink-300"

    def test_general_chip_dark_bg(self):
        src = _src_model_picker()
        assert "dark:bg-blue-900/30" in src, "General chip must include dark:bg-blue-900/30"

    def test_general_chip_dark_text(self):
        src = _src_model_picker()
        assert "dark:text-blue-300" in src, "General chip must include dark:text-blue-300"
...
class TestModelCheckboxRowDarkMode:
    ...
    def test_model_name_dark_text(self):
        src = _src_model_picker()
        assert "dark:text-gray-100" in src, "Model display name must include dark:text-gray-100"

    def test_model_description_dark_text(self):
        src = _src_model_picker()
        # text-gray-500 dark:text-gray-400 for description
        assert "dark:text-gray-400" in src, "Model description must include dark:text-gray-400"

    def test_cached_badge_dark(self):
        src = _src_model_picker()
        assert "dark:bg-green-900/30" in src, "Cached badge must include dark:bg-green-900/30"
        assert "dark:text-green-300" in src, "Cached badge must include dark:text-green-300"
...
class TestAnalysisControlsDarkMode:
    ...
    def test_no_slide_msg_dark(self):
        ...
        assert "dark:text-gray-400" in src, "'Select a case' text must include dark:text-gray-400"
    ...
    def test_embedding_progress_text_dark(self):
        src = _src_analysis_controls()
        assert "dark:text-violet-300" in src, "Embedding title must include dark:text-violet-300"
        assert "dark:text-violet-400" in src, "Embedding message must include dark:text-violet-400"
    ...
    def test_amber_warning_box_dark(self):
        src = _src_analysis_controls()
        assert "dark:bg-amber-900/30" in src, "Amber warning box must include dark:bg-amber-900/30"
        assert "dark:border-amber-700" in src, (
            "Amber warning box must include dark:border-amber-700"
        )
        assert "dark:text-amber-400" in src, "Amber warning text must include dark:text-amber-400"

    def test_run_analysis_button_dark(self):
        src = _src_analysis_controls()
        assert "dark:bg-sky-900/30" in src, "Run Analysis button must include dark:bg-sky-900/30"
        assert "dark:text-sky-200" in src, "Run Analysis button must include dark:text-sky-200"
        assert "dark:border-sky-600" in src, "Run Analysis button must include dark:border-sky-600"
    ...
    def test_no_model_warning_dark(self):
        ...
        assert count >= 2, f"Amber warning texts should have dark:text-amber-400 (found {count})"
...
class TestLightModePreserved:
    ...
    def test_badge_keeps_light_gradients(self):
        ...
        for color in ["green", "amber", "red", "blue", "clinical"]:
            assert f"from-{color}-50" in src, f"Badge must retain from-{color}-50 for light mode"
