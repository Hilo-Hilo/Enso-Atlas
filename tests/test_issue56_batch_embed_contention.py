...
def test_backend_analyze_multi_returns_server_busy_contract_when_batch_embed_active():
    ...
    assert "active_batch_embedding = active_batch_embed_info()" in src
...
def test_health_endpoint_exposes_active_batch_embed_and_uses_cached_cuda_probe():
    main_src = _read("src/enso_atlas/api/main.py")

    assert "_CUDA_AVAILABLE_AT_STARTUP" in main_src
    assert '"cuda_available": bool(_CUDA_AVAILABLE_AT_STARTUP)' in main_src
    assert '"active_batch_embedding": active_batch_embedding' in main_src
