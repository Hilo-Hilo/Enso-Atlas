...
class AgentState:
    ...
    # Analysis context
    context: AnalysisContext = field(default_factory=lambda: AnalysisContext(slide_id=""))
...
class AgentWorkflow:
    ...
    def __init__(
        ...
        slide_labels: dict[str, str] | None = None,
        slide_mean_index: Any = None,
        slide_mean_ids: list[str] | None = None,
        slide_mean_meta: dict[str, dict[Any, Any]] | None = None,
    ):
        ...
        try:
            ...
            if attention is not None:
                ...
                for i, idx in enumerate(top_indices):
                    ...
                    if state.context.coordinates is not None and idx < len(
            ...
            result = StepResult(
                ...
                data={
                    "predictions": {
                        k: {
                            key: v[key]
                            for key in ["model_name", "label", "score", "confidence"]
                            if key in v
                        }
    ...
    async def _step_retrieve(self, state: AgentState) -> StepResult:
        ...
        if state.context.embeddings is None:
            duration = (time.time() - start) * 1000
            result = StepResult(
                step=AgentStep.RETRIEVE,
                status=StepStatus.ERROR,
                message="Embeddings not loaded",
                reasoning="Cannot retrieve similar cases before initialization loads embeddings.",
                duration_ms=duration,
            )
        ...
        try:
            ...
            non_responders = sum(
                1 for c in state.similar_cases if c.get("label") == "non-responder"
            )
    ...
    async def _step_compare(self, state: AgentState) -> StepResult:
        ...
        reasoning = "Comparison with similar cases:\n" + "\n".join(
            f"- {i}" for i in comparison_insights
        )
        state.reasoning_chain.append(reasoning)
        ...
        if state.predictions:
            for model_id, pred in state.predictions.items():
                if "error" not in pred:
                    confidence = pred.get("confidence", 0)
                    level = (
                        "high" if confidence > 0.6 else "moderate" if confidence > 0.3 else "low"
                    )
                    reasoning_parts.append(
    ...
    def _generate_answer(self, question: str, state: AgentState) -> str:
        ...
        if any(
            word in q_lower
            for word in ["response", "treatment", "platinum", "sensitive", "resistant", "chemo"]
        ):
            ...
            if pred is not None:
                ...
                if state.similar_cases:
                    resp_count = sum(
                        1 for c in state.similar_cases if c.get("label") == "responder"
                    )
        ...
        if any(
            word in q_lower for word in ["survival", "prognosis", "outcome", "year", "mortality"]
        ):
            ...
            for model_id in survival_models:
                if model_id in state.predictions:
                    ...
                    responses.append(
                        f"• **{years} survival**: {outcome} ({score:.1%} positive probability)"
                    )
        ...
        if any(word in q_lower for word in ["compare", "similar", "cases", "other", "like this"]):
            if state.similar_cases:
                ...
                responders = [c for c in state.similar_cases if c.get("label") == "responder"]
                non_responders = [
                    c for c in state.similar_cases if c.get("label") == "non-responder"
                ]
        ...
        if any(word in q_lower for word in ["alternative", "other treatment", "instead", "option"]):
            ...
            if treatment_pred is not None:
                ...
                is_negative = (
                    "non" in label or "resistant" in label or "negative" in label or "poor" in label
                )
...
⚠️ This is a research tool — consult with the clinical team for treatment planning."""
        ...
        if any(word in q_lower for word in ["confident", "certain", "sure", "reliable", "trust"]):
            if state.predictions:
                ...
                response += "\n**Interpretation:**\n"
                avg_conf = sum(
                    p.get("confidence", 0) for p in state.predictions.values() if "error" not in p
                ) / max(1, len(state.predictions))
                if avg_conf > 0.6:
        ...
        if any(word in q_lower for word in ["differential", "diagnos", "possibilities"]):
            ...
            if treatment_pred is not None:
                ...
                if is_negative:
                    ...
                    response += (
                        "• **Homogeneous tumor architecture**: Consistent cellular patterns\n"
                    )
                    response += (
                        "• **Typical cancer morphology**: Characteristic histological features\n"
                    )
...
Please rephrase your question using one of these topics."""
    ...
    def get_result(self, session_id: str) -> AgentResult | None:
        """Get the final result for a completed session."""
        state = self._sessions.get(session_id)
        if state is None:
            return None

        total_duration = sum(r.duration_ms for r in state.step_results.values())
