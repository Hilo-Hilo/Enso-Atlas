...
def create_header_footer(canvas_obj, doc):
    ...
    canvas_obj.drawString(
        20 * mm, page_height - 26 * mm, "Enso Atlas | AI-Assisted Diagnostic Platform"
    )

    # Date on right
    canvas_obj.drawRightString(
        page_width - 20 * mm, page_height - 18 * mm, datetime.now().strftime("%Y-%m-%d %H:%M")
    )
...
def generate_pdf_report(
    ...
    info_data = [
        ...
        [
            "Generated:",
            datetime.fromisoformat(generated_at.replace("Z", "+00:00")).strftime(
                "%Y-%m-%d %H:%M UTC"
            )
            if generated_at
            else "N/A",
    ...
    if decision_support:
        ...
        if primary_rec:
            story.append(
                Paragraph(f"<b>Primary Recommendation:</b> {primary_rec}", styles["BodyText"])
            )
    ...
    if evidence:
        ...
        for i, item in enumerate(evidence[:5], 1):
            ...
            story.append(
                Paragraph(f"<b>Evidence #{i}</b> - Coordinates: {coords_str}", styles["SubHeader"])
            )
    ...
    if evidence_patches:
        ...
        story.append(
            Paragraph(
                "High-attention patches that influenced the model prediction:", styles["SmallText"]
            )
        )
        story.append(Spacer(1, 3 * mm))

        # Create grid of patch images (3 per row)
        patch_row: list[list[Any]] = []
        for i, patch in enumerate(evidence_patches[:9]):  # Max 9 patches
            if patch.get("image"):
                try:
                    ...
                    cell_content = [rl_img, Paragraph(caption, styles["SmallText"])]
        ...
        if patch_row:
            while len(patch_row) < 3:
                patch_row.append([Paragraph("", styles["SmallText"])])
            row_table = Table([patch_row], colWidths=[55 * mm, 55 * mm, 55 * mm])
            row_table.setStyle(
                TableStyle(
                    [
            ...
            TableStyle(
                [
                    ...
                    (
                        ...
                        (0, 1),
                        (-1, -1),
                        [colors.white, colors.Color(248 / 255, 248 / 255, 248 / 255)],
                    ),
...
def generate_report_pdf(report: dict[str, Any], case_id: str) -> bytes:
    ...
    class _ClinicalPDF(FPDF):
        ...
        def header(self):
            ...
            self.cell(
                0, 5, "AI-Assisted Diagnostic Platform", align="C", new_x="LMARGIN", new_y="NEXT"
            )
            self.ln(6)
        ...
        def footer(self):
            ...
            self.cell(0, 10, f"{FOOTER_TEXT}  |  Page {self.page_no()}/{{nb}}", align="C")
    ...
    y_start = pdf.get_y()
    pdf.rect(pdf.l_margin, y_start, pdf.w - pdf.l_margin - pdf.r_margin, 14, style="DF")
    pdf.set_xy(pdf.l_margin + 2, y_start + 2)
    pdf.set_font("Helvetica", "B", 9)
    pdf.set_text_color(153, 102, 0)
    pdf.cell(0, 5, "UNCALIBRATED MODEL - RESEARCH USE ONLY", new_x="LMARGIN", new_y="NEXT")
    ...
    generated_at = report.get(
        "generated_at", report.get("generatedAt", datetime.now().strftime("%Y-%m-%d %H:%M"))
    )
    ...
    if model_output:
        ...
        prob = model_output.get("probability", model_output.get("score", 0))
        ci = model_output.get("confidence_interval", model_output.get("confidenceInterval"))
        ...
        cal_note = model_output.get("calibration_note", model_output.get("calibrationNote", ""))
        if cal_note:
    ...
    if evidence_items:
        ...
        for i, item in enumerate(evidence_items[:10], 1):
            ...
            morphology = item.get(
                "morphology_description", item.get("morphologyDescription", "N/A")
            )
            significance = item.get("significance", item.get("whyThisPatchMatters", ""))
            attention = item.get("attention_weight", item.get("attentionWeight"))
            ...
            pdf.cell(0, 6, "  |  ".join(header_parts), new_x="LMARGIN", new_y="NEXT")
            pdf.set_text_color(30, 30, 30)
    ...
    similar = report.get("similar_cases", report.get("similarExamples", []))
    if similar:
        _section_heading("Similar Cases")
        # Table header
        col_w = [(pdf.w - pdf.l_margin - pdf.r_margin) * r for r in (0.45, 0.30, 0.25)]
    ...
    decision = report.get("decision_support", report.get("decisionSupport"))
    if decision:
        _section_heading("Clinical Decision Support")
        rec = decision.get("primary_recommendation", decision.get("primaryRecommendation", ""))
        if rec:
            _label_value("Recommendation:", rec)
        rationale = decision.get("supporting_rationale", decision.get("supportingRationale", []))
        if rationale:
            pdf.set_font("Helvetica", "B", 9)
            pdf.set_text_color(60, 60, 60)
            ...
            for r in rationale:
                _bullet(r)
        workup = decision.get("suggested_workup", decision.get("suggestedWorkup", []))
        if workup:
            pdf.set_font("Helvetica", "B", 9)
            pdf.set_text_color(60, 60, 60)
    ...
    # ---- Suggested Next Steps -------------------------------------------
    next_steps = report.get("suggested_next_steps", report.get("suggestedNextSteps", []))
    if next_steps:
    ...
    # ---- Safety Statement -----------------------------------------------
    safety = report.get("safety_statement", report.get("safetyStatement", ""))
    if safety:
