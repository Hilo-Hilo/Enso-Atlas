...
class SlideMetadataManager:
    ...
    def _load(self) -> None:
        ...
        if self.storage_path.exists():
            try:
                ...
                logger.info(
                    f"Loaded slide metadata: {len(self._store.slides)} slides, {len(self._store.groups)} groups"
                )
    ...
    def get_all_tags(self) -> AllTagsResponse:
        ...
        with self._lock:
            ...
            return AllTagsResponse(tags=sorted(tag_counts.keys()), tag_counts=tag_counts)
    ...
    def list_groups(self) -> list[GroupResponse]:
        """Return group summaries for route handlers."""
    ...
    def create_group(
        ...
        with self._lock:
            # Generate unique ID
            group_id = f"grp_{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S%f')}"
    ...
    def get_slide_metadata(self, slide_id: str) -> SlideMetadataResponse | None:
    ...
    def update_metadata(
        self, slide_id: str, notes: str | None = None, custom_metadata: dict[str, Any] | None = None
    ) -> SlideMetadata:
    ...
    def get_starred_slides(self) -> list[str]:
        ...
        with self._lock:
            return [slide.slide_id for slide in self._store.slides.values() if slide.starred]
...
def create_metadata_router(
    manager: SlideMetadataManager, get_available_slides: Callable[[], list[str]]
) -> APIRouter:
    ...
    async def update_slide_metadata(
        slide_id: str, notes: str | None = None, custom_metadata: dict[str, Any] | None = None
    ):
        """Update slide notes and custom metadata."""
