...
class ChatSession:
    ...
    def add_message(self, role: str, content: str, evidence_patches: list | None = None) -> Message:
...
class ChatManager:
    ...
    def get_or_create_session(
        self, session_id: str | None = None, slide_id: str | None = None
    ) -> ChatSession:
    ...
    def _generate_response(
        self, question: str, context: ChatContext, history: list[Message]
    ) -> str:
        """
        ...
        if any(
            word in q_lower
            for word in ["prognosis", "survival", "outcome", "life expectancy", "how long"]
        ):
            survival_models = ["survival_5y", "survival_3y", "survival_1y"]
            responses = []

            for model_id in survival_models:
                if model_id in context.predictions and "error" not in context.predictions.get(
                    model_id, {}
                ):
                    ...
                    responses.append(
                        f"• **{years} survival**: {emoji} {outcome} ({score:.1%} positive probability)"
                    )
            ...
            if responses:
                ...
                if context.similar_cases:
                    resp_count = sum(
                        1 for c in context.similar_cases if c.get("label") == "responder"
                    )
                ...
                result += (
                    "Final prognosis should consider stage, molecular markers (BRCA/HRD status), "
                )
...
⚠️ This is a morphology-based assessment only. Comprehensive prognostic evaluation requires integration of clinical staging, molecular profiling, and multidisciplinary tumor board review."""
            ...
            return (
                "Survival models not available for this analysis. Please run a full analysis first."
            )

        # === TREATMENT RESPONSE QUESTIONS ===
        if any(
            word in q_lower
            for word in ["treatment", "response", "platinum", "chemo", "sensitive", "resistant"]
        ):
            ...
            treatment_pred: dict[str, Any] | None = None
            ...
            for key in treatment_keys:
                if key in context.predictions and "error" not in context.predictions.get(key, {}):
                    treatment_pred = context.predictions[key]
                    pred_model_name = treatment_pred.get(
            ...
            if treatment_pred is None:
                for v in context.predictions.values():
                    if "error" not in v:
                        treatment_pred = v
                        pred_model_name = treatment_pred.get(
                            "model_name", "Treatment Response"
                        )
                        break

            if treatment_pred is not None:
                if "error" in treatment_pred:
                    return f"Error in {pred_model_name} model: {treatment_pred.get('error')}"

                score = treatment_pred.get("score", 0.5)
                label = treatment_pred.get("label", "unknown")
                confidence = treatment_pred.get("confidence", 0)

                response = f"**{pred_model_name} Prediction:**\n\n"
                response += (
                    f"The model predicts **{label.upper()}** with {score:.1%} probability.\n\n"
                )
        ...
        if any(
            word in q_lower
            for word in ["evidence", "region", "area", "attention", "patch", "show me", "where"]
        ):
            if context.top_evidence:
                response = (
                    f"🔍 **High-Attention Regions ({len(context.top_evidence)} identified):**\n\n"
                )
        ...
        if any(word in q_lower for word in ["similar", "compare", "other cases", "like this"]):
            if context.similar_cases:
                ...
                non_responders = [
                    c for c in context.similar_cases if c.get("label") == "non-responder"
                ]
                ...
                for c in context.similar_cases[:5]:
                    ...
                    emoji = (
                        "🟢" if label == "responder" else "🟠" if label == "non-responder" else "⚪"
                    )
                    response += (
                        f"• {emoji} `{c['slide_id']}` — {sim * 100:.0f}% similarity ({label})\n"
                    )
        ...
        if any(
            word in q_lower for word in ["confidence", "certain", "reliable", "trust", "accurate"]
        ):
            if context.predictions:
                ...
                if count > 0:
                    ...
                    if avg_conf > 0.6:
                        ...
                        response += (
                            "Low confidence. Recommend molecular testing and expert consultation."
                        )
...
Please select a slide and run analysis first if you haven't already."""
    ...
    async def chat(
        ...
        if session.context and any(
            word in q_lower for word in ["region", "evidence", "show", "where", "area", "attention"]
        ):
