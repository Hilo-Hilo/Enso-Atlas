...
import { getProjectFallbackModels } from "./ModelPicker";
...
function ModelCard({ prediction, isExpanded, onToggle }: ModelCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Expanded details */}
      {isExpanded && (
        <div className="px-3 pb-3 space-y-3 border-t border-gray-100 bg-white">
...
export function MultiModelPredictionPanel({
  ...
  const projectFallbackModels = React.useMemo(
    () => getProjectFallbackModels(currentProject),
    [currentProject]
  );

  // Prefer project API models, then project-derived fallback
