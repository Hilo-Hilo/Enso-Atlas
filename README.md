...
- 🎯 **Evidence-based**: Heatmaps + patch retrieval + structured reports
- 🔄 **Foundation-model agnostic**: Swap embedders without changing the product
- ⚡ **Efficient**: Embed once, reuse for multiple downstream tasks

## Architecture

```
WSI → Patches → Path Foundation → Attention MIL → Heatmap + FAISS → MedGemma Report
```
...
med-gemma-hackathon/
├── src/
│   ├── wsi/           # WSI processing & tiling
│   ├── embedding/     # Path Foundation feature extraction
│   ├── mil/           # CLAM attention MIL head
│   ├── evidence/      # Heatmaps & FAISS retrieval
